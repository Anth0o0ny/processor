...
#
# from isa import ALUOpcode, Opcode, Selectors, nullar_instructions, onear_instructions, read_code
#
...
#         elif self.operation == ALUOpcode.AND:
#             tmp_result = self.src_a & self.src_b
...
#                 ALUOpcode.AND, left_sel=Selectors.FROM_AC, right_sel=Selectors.FROM_DR
#             )
#             self.tick()
...
#         if not input_text:
#             return tokens  # Возвращаем пустой список, если ввод пуст
#         else:
#             input_tokens = eval(input_text)
#
#     if len(input_tokens) > 0:
#         for symbol in input_tokens:
...
class ControlUnit:
    ...
    def check_for_interruptions(self, enabled: bool = False) -> bool:
        ...
        if not enabled or self.current_tick() < len(self.data_path.input_buffer):
            return False
        return True
    ...
    def __repr__(self) -> str:
        return "AC: {:4} | PC: {:3} | IR: {:5} | DR: {:7} | SP: {:3} | Addr: {:3} | ToMem: {:7} | N: {:1} | Z: {:1} | INT_EN: {:1} | mem[Addr]: {:7} | mode: {}".format(
            self.data_path.ac,
            self.data_path.pc,
...
def simulation(code: list, input_symbols: list, memory_size: int, limit: int) -> tuple:
    data_path = DataPath(memory_size, input_symbols)
    control_unit = ControlUnit(code, data_path)
    instr_counter = 0
    ...
    if len(input_tokens) > 0:
        for item in input_tokens:
            if isinstance(item, tuple) and len(item) == 2:
                tokens.append(item[1])  # Append the symbol part of the tuple
            else:
                tokens.append(item)  # Append the entire item if it's not a tuple
    return tokens
...
def main(code_file: str, input_file: str):
    ...
    output_symbols, output_numbers, instr_counter, ticks = simulation(
        code,
        input_token,  # Modified this line to pass input tokens directly
        memory_size=200,
        limit=5000,
    )
